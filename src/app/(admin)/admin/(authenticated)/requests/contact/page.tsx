...
export const metadata = { title: "Контактные заявки — Эрфольг" };
...
export default async function ContactRequestsPage(
  ...
  let dbError = false;
  try {
    rows = await db.contactRequest.findMany({
      where: status
    ...
    console.error("contact requests fetch error", e);
    dbError = true;
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Сообщения с формы «Связаться»
        </p>
      </div>
      <RequestsTabs current="contact" />
      <RequestStatusFilter base="/admin/requests/contact" status={status} />
      {dbError ? (
            ...
            <TableBody>
              {rows.map((r) => (
                <TableRow key={r.id}>
                  <TableCell>
                    <Link
                      href={`/admin/requests/contact/${r.id}`}
